...
class AI_Spell extends AI_Game_content
{
	...
	public function valid_version()
	{
		...
		{
		case LASER_SHOT:
			if($this->effect != '%power instant damage' OR
				$this->codename != 'LASER_SHOT')
				return FALSE;
			break;
		case OFFENSIVE_BOTS:
			if($this->effect != '%power damage over %duration turns' OR
		...
		case EXPLOSIVE_DEVICE;
			if($this->effect != '%power instant damage, and for %duration turns provide %energy extra energy when you use this spell again' OR
		...
		case OFFENSIVE_SYSTEM_VIRUS;
			...
			break;
		case REPARING;
			if($this->effect != 'instant %power health' OR
				$this->codename != 'REPARING')
				return FALSE;
			break;
		case REPARING_BOTS;
			if($this->effect != '%power health over %duration turns' OR
		...
		case MOBILE_REPAIR_ROBOT;
			if($this->effect != 'instant %power health and for %duration turns provide %energy extra energy when you use this spell again' OR
